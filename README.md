...
- `GitTool-262-linux-x64-installer.sh` (~330 MB) -- самораспаковывающийся
  установщик (собирается `scripts\make-release-linux.ps1` через WSL, чтобы
  сохранить unix-права). Установка: `sh GitTool-262-linux-x64-installer.sh`
  (опционально: префикс первым аргументом; по умолчанию `~/.local/share`).
  Ставит в `~/.local/share/GitTool`, создаёт симлинк `~/.local/bin/gittool`
  и `.desktop`-ярлык, преднастраивает Classic UI и урезанные меню в
  `~/.config/GitTool2026.2`.
- git на Linux берётся системный (`apt/dnf install git`), JBR внутри пакета.
