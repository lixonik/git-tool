...
- `GitTool-262-setup.exe` (~224 MB) -- самораспаковывающийся установщик
  (7z SFX): распаковывается, копирует GitTool в `%LOCALAPPDATA%\GitTool`,
  создаёт ярлык на рабочем столе и запускает. Один файл -- можно переносить
  на любой Windows x64.
- `GitTool-262-portable.7z` -- portable-вариант: распаковать куда угодно,
  запускать `GitToolMini\GitTool.bat`. Все настройки (config/system) живут
  внутри папки -- инструмент можно таскать вместе с настройками.

Нюансы переноса:

- exe не подписан: на чужом ПК SmartScreen покажет предупреждение
  («Подробнее» -> «Выполнить в любом случае»).
- Требуется Windows x64; JBR (Java-рантайм) внутри, ставить ничего не нужно.
- git.exe должен быть на PATH целевой машины (сам git в пакет не входит).
