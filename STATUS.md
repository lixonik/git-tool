...
- В индексе git у D:\Repos\intellij-community ~273k staged deletions --
  артефакт прерванной git-операции. Рабочее дерево целое, сборке не мешает.
  Лечится (при желании) git reset --mixed HEAD; я чужой индекс не трогал.
...
- Попытка 6 (идёт): с IPv4-флагом и полным пре-сидом.
...
2. Скачать каждый файл PowerShell-ом с оригинального хоста (не через
   cache-redirector), для maven-артефактов сверить .sha1.
